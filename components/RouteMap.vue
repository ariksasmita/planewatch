...
onMounted(async () => {
  ...
  if (props.live) {
    const livePosition: [number, number] = [props.live.latitude, props.live.longitude]
    ...
    L.marker(livePosition, { icon: planeIcon })
      .addTo(map)
      .bindPopup(`
        <div style="font-size:12px">
          <strong>Live ADS-B position</strong><br/>
          Altitude: ${Math.round(props.live.altitude).toLocaleString()} ft<br/>
          Speed: ${Math.round(props.live.speed_horizontal).toLocaleString()} kt<br/>
